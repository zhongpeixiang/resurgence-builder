      ...
      : item.type === 'Weapon'
        ...
        : item.type === 'Talent'
          ...
          : item.type === 'Brand'
            ...
            : [item.name, item.core, item.rarity, item.talent, item.description, item.secondaryDescription, item.mainStat?.label, item.mainStat?.value, ...item.attributes.flatMap(attribute => [attribute.label, attribute.value])];
...
if (typeof document !== 'undefined') {
  ...
  function gearCard(item) { return `<article class="item-card gear-card"><div class="gear-summary"><p class="eyebrow"><span class="tag">${item.slot}</span><span class="tag ghost">${item.tier}</span></p><h3>${item.name}</h3><p class="tags">${item.modSlots.map(tag => `<span>${tag}</span>`).join('')}</p></div><div class="gear-detail gear-stats"><p><small>${item.fact.label}</small><b>${item.fact.value}</b><em>${item.fact.note}</em></p><p class="tags">${item.brands.map(brand => `<span>${brand}</span>`).join('') || '<span>No brand bonuses</span>'}</p></div><section class="weapon-talent-pool gear-talent-pool" aria-label="Compatible talents for ${item.name}"><header><div><small>COMPATIBLE TALENT POOL</small><b>${item.talents.length} sourced options</b></div><em>Choose this gear piece, then select a compatible talent</em></header><div class="talent-pool-list"><section class="talent-pool"><small>${item.slot} talents</small><p class="tags">${item.talents.map(talent => `<span>${talent}</span>`).join('') || '<span>—</span>'}</p></section></div></section><button class="add" data-item="${item.id}" data-target="${item.slot}">Equip ${item.slot}</button></article>`; }
  function weaponCard(item) {
    ...
    return `<article class="item-card weapon-card"><div class="weapon-summary"><p class="eyebrow"><span class="tag">${item.weaponClass}</span><span class="tag ghost">${item.damageType}</span></p><h3>${item.name}</h3><p class="tags">${item.badges.map(tag => `<span>${tag}</span>`).join('')}<span>BURST DPS: ${dps.burst}</span><span>SUSTAINED DPS: ${dps.sustained}</span><span>ACCURACY: ${handling.accuracy}</span><span>STABILITY: ${handling.stability}</span></p></div><div class="gear-detail weapon-stats"><p><small>${item.facts[0]?.label ?? '—'}</small><b>${item.facts[0]?.value ?? '—'}</b><em>${item.facts[0]?.note ?? ''}</em></p><p class="tags">${item.facts.slice(1, 3).map(fact => `<span>${fact.label}: ${fact.value}</span>`).join('')}</p></div><section class="weapon-talent-pool" aria-label="Compatible talents for ${item.name}"><header><div><small>COMPATIBLE TALENT POOL</small><b>${item.talents.length} sourced options</b></div><em>Choose one from each pool in the builder</em></header><div class="talent-pool-list">${talentPools.map(pool => `<section class="talent-pool"><small>${pool.slot}</small><p class="tags">${pool.talents.map(talent => `<span>${talent.name}</span>`).join('')}</p></section>`).join('')}</div></section><div class="weapon-actions"><button class="add" data-item="${item.id}" data-target="Primary Weapon">Equip primary</button><button class="add" data-item="${item.id}" data-target="Secondary Weapon">Equip secondary</button></div></article>`;
  }
  ...
  function protocolCard(item) { return `<article class="item-card protocol-card"><div><p class="eyebrow"><span class="tag">${item.core}</span><span class="tag ghost">${item.rarity}</span></p><h3>${item.name}</h3><p class="talent-copy">${item.talent ?? item.description ?? 'No talent supplied.'}</p></div><div class="gear-detail"><p><small>MAIN STAT</small><b>${item.mainStat ? `${item.mainStat.label} ${item.mainStat.value}` : '—'}</b><em>${item.cooldown ? `Cooldown: ${item.cooldown}` : 'No cooldown'}</em></p><p class="tags">${item.attributes.map(attribute => `<span>${attribute.label}: ${attribute.value}</span>`).join('') || '<span>No additional attributes</span>'}</p></div><button class="add" data-item="${item.id}" data-target="OS Protocol">Equip OS protocol</button></article>`; }
