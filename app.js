...
if (typeof document !== 'undefined') {
  ...
  function renderDatabase() {
    ...
    items.innerHTML = visible.map(item => item.type === 'Gear' ? gearCard(item) : item.type === 'Weapon' ? weaponCard(item) : item.type === 'Talent' ? talentCard(item) : item.type === 'Brand' ? brandCard(item) : protocolCard(item)).join('') || '<p class="empty">No equipment matches this search.</p>';
  }
