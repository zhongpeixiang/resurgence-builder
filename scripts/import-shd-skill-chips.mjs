...
const records = Function(`return (${source.slice(start, end)})`)().map(item => ({ id:item.id, name:item.name, type:'Skill Chip', kicker:item.kicker, iconUrl:item.iconUrl ?? '', badges:(item.badges ?? []).map(badge => badge.label), facts:item.facts ?? [], lines:item.lines ?? [], description:item.description ?? '', attributes:item.afterLines ?? [], source:'SHD.build database/skill-chips page provided by authorized contributor' }));
